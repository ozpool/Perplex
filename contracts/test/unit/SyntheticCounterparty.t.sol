...
contract SyntheticCounterpartyTest is Test {
    ...
    function test_constructor_acceptsZeroSettlement() public {
        SyntheticCounterparty c = new SyntheticCounterparty(owner, address(0), address(usdc), address(vault));
    ...
    function test_onFill_capExceeded_reverts() public {
        ...
        vm.prank(settlement);
        vm.expectRevert(abi.encodeWithSelector(ISyntheticCounterparty.CapExceeded.selector, BTC, 5e18, 6e18));
        cp.onFill(address(cp), BTC, 2e18, 100_000e18, 0);
    }
    ...
    function test_onFill_perMarketCapsAreIndependent() public {
        ...
        vm.prank(settlement);
        vm.expectRevert(abi.encodeWithSelector(ISyntheticCounterparty.CapExceeded.selector, BTC, 1e18, 2e18));
        cp.onFill(address(cp), BTC, 1e18, 100_000e18, 0);
